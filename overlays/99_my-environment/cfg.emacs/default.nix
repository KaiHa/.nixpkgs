...
let
  emacsService = writeText "emacs.service" ''
    ...
    Type=notify
    ExecStart=${bash}/bin/bash -c 'source /etc/profile; PATH="$PATH:${graphicsmagick-imagemagick-compat}/bin:${mediainfo}/bin:${rtags}/bin:${weechat}/bin" exec ${emacs}/bin/emacs --fg-daemon'
